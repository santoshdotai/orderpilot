...
export function QuotationsPage() {
  ...
  const visibleQuotations = safeArray<Quotation>(quotations);
  ...
  async function handleApprove(quotation: Quotation) {
    try {
      ...
      await approveAndSendQuotation(quotation);
  ...
  return (
    <>
      ...
      {loading && !visibleQuotations.length ? <LoadingState label="Loading quotation drafts and line items..." /> : null}
      {successMessage ? (
      ...
      {!loading && !error && !visibleQuotations.length ? (
        <EmptyState
          title="No quotation drafts yet"
          description="Send a WhatsApp order through the Interakt intake flow and let Phase 5.2 create the first draft quotation."
        />
      ) : null}

      <div className="grid gap-4">
        {visibleQuotations.map((quotation) => {
          const items = safeArray<QuotationItem>(quotation.items);
